...
public class test {
	...
	String output_path = "simulation_me.txt";
		...
		int PC = 256;
		int[] Data_address = new int[2];
		int[] Data_end_address = new int[2];
		...
		*/
		...
		while (is_go[0]) {
			...
			MEM_cache = MEM(Pre_MEM_QUEUE, Post_MEM_QUEUE, InstrSet, Data, GPR, Data_address[0], element_del, update);

			ALU1(InstrSet, Pre_ALU1_QUEUE, Pre_MEM_QUEUE, Data, GPR, Data_address[0], SW_Write_done);

			ALU2_cache = ALU2(InstrSet, Pre_ALU2_QUEUE, Post_ALU2_QUEUE, GPR, Data);

			issue_sum = Exe_ISSUE(InstrSet, Pre_ISSUE_QUEUE, Pre_ALU1_QUEUE, Pre_ALU2_QUEUE, GPR_state, ALU1_instru,
					ALU2_instru, ISSUED_INSTRU, SW_Write_done);
			...
			ShowQueues(output_path, Cycle, deassems, Waiting_QUEUE, Executed_QUEUE, Pre_ISSUE_QUEUE, Pre_ALU1_QUEUE,
					Pre_ALU2_QUEUE, Pre_MEM_QUEUE, Post_MEM_QUEUE, Post_ALU2_QUEUE,1);
			ShowResult(output_path, Cycle, GPR_state, GPR, Data, Data_address,1);
	...
	private static int Exe_ISSUE(String[] InstruSet, QUEUE Pre_ISSUE_QUEUE, QUEUE Pre_ALU1_QUEUE, QUEUE Pre_ALU2_QUEUE,
			String[] GPR_state, int[] ALU1_instru, int[] ALU2_instru, QUEUE ISSUED_INSTRU, boolean[] SW_Write_done)
			throws Exception {
		...
		while (!Pre_ISSUE_QUEUE.isEmpty()) {
			...
			while (!Issue_temp.isEmpty()) {
				...
				if (Harzed_WAW(Instru_1_addr, InstruSet, instr_temp)
						|| Harzed_WAR(Instru_1_addr, InstruSet, instr_temp, SW_Write_done)
						|| Harzed_WAR(instr_temp, InstruSet, Instru_1_addr, SW_Write_done)) {
					flag = true;
					break;
				}
			}
	...
	private static int[] ALU1(String[] InstruSet, QUEUE Pre_ALU1_QUEUE, QUEUE Pre_MEM_QUEUE, int[] Data, int[] GPR,
			int Data_address, boolean[] SW_Write_done) throws Exception {
	...
	private static int[][] WriteBack(int[] MEM_cache, int[] ALU2_cache, QUEUE Post_MEM_QUEUE, QUEUE Post_ALU2_QUEUE,
			int[] GPR, int[] Data, String[] GPR_state, QUEUE ISSUED_INSTRU, int[] element_del, boolean[] update)
			throws Exception {
	...
	private static void Delete_element(QUEUE ISSUED_INSTRU, int[] element_del, String[] Reg_state, int[][] update_cache)
			throws Exception {
		// TODO Auto-generated method stub
		QUEUE temp = new QUEUE(20);
	...
	private static boolean Harzed_WAR(int Instr_1, String[] InstruSet, int Instr_2, boolean[] SW_Write_done)
			throws Exception {
	...
	private static void ShowResult(String fileName, int cycle, String[] GPR_state, int[] GPR, int[] Data,
			int[] data_addr, int show_type) throws IOException {
		/*
		 * // show reg_state System.out.println(); System.out.println("GPR"); for (int i
		 * = 0; i < GPR.length; i++) { if (i % 8 == 0) { System.out.println(""); }
		...
		for (int i = 0; i < GPR.length; i++) {
			...
			writer.write('\t' + String.valueOf(GPR[i]));
			if ((i + 1) % 8 == 0 && i < GPR.length - 1) {
				writer.write('\n');
			}
		}
		writer.write('\n');
	...
	private static void ShowQueues(String fileName, int cycle, String[] deassems, QUEUE waiting_QUEUE,
			QUEUE executed_QUEUE, QUEUE pre_ISSUE_QUEUE, QUEUE pre_ALU1_QUEUE, QUEUE pre_ALU2_QUEUE,
			QUEUE pre_MEM_QUEUE, QUEUE post_MEM_QUEUE, QUEUE post_ALU2_QUEUE, int show_type) throws Exception {
		...
		writer.write("\nIF Unit:");
		if (!waiting_QUEUE.isEmpty()) {
			writer.write("\n\tWaiting Instruction: " + "[" + deassems[waiting_QUEUE.peek()].substring(37) + "]");
		...
		if (!executed_QUEUE.isEmpty()) {
			writer.write("\n\tExecuted Instruction: " + "[" + deassems[executed_QUEUE.peek()].substring(37) + "]");
			// System.out.println(deassems[waiting_QUEUE.peek()].substring(37));
		...
		int num = 0;
		while (!pre_ISSUE_QUEUE.isEmpty()) {
			writer.write("\n\tEntry " + num++ + ": " + "[" + deassems[pre_ISSUE_QUEUE.peek()].substring(37) + "]");
		...
		while (!pre_ALU1_QUEUE.isEmpty()) {
			writer.write("\n\tEntry " + num++ + ": " + "[" + deassems[pre_ALU1_QUEUE.peek()].substring(37) + "]");
		...
		while (!pre_MEM_QUEUE.isEmpty()) {
			writer.write(" [" + deassems[pre_MEM_QUEUE.peek()].substring(37) + "]");
		...
		while (!post_MEM_QUEUE.isEmpty()) {
			writer.write(" [" + deassems[post_MEM_QUEUE.peek()].substring(37) + "]");
		...
		while (!pre_ALU2_QUEUE.isEmpty()) {
			writer.write("\n\tEntry " + num++ + ":" + " [" + deassems[pre_ALU2_QUEUE.peek()].substring(37) + "]");
		...
		while (!post_ALU2_QUEUE.isEmpty()) {
			writer.write(" [" + deassems[post_ALU2_QUEUE.peek()].substring(37) + "]");
		...
		while (!post_MEM_QUEUE.isEmpty()) {
			System.out.println(deassems[post_MEM_QUEUE.peek()].substring(37));
			show_temp.push(post_MEM_QUEUE.pop());
		}
		while (show_temp != null && !show_temp.isEmpty()) {
			post_MEM_QUEUE.push(show_temp.pop());
